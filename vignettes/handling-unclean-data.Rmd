...
# Unfortunately, error messages refer to internal colums unknown to the user
try(f(tibble(xxx = 1), xxx))
```
...
* And for multiple groups passed via `...`, you'll need to capture them with `rlang::enquos()`, then clean them with `clean_quos()` and unquote-splice it with big bang `!!!`:
